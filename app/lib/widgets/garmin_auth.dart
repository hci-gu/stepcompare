    ...
    return Padding(
      ...
      child: Column(
        children: [
          ...
          TextButton.icon(
            onPressed: () async {
              GarminClient client = GarminClient(username.text, password.text);
              await client.connect();
              String today = DateTime.now().toIso8601String().substring(0, 10);
              List<Map<String, dynamic>> data = await client.fetchSteps(today);
              data.forEach((element) {
                print('${element['date_from']} - ${element['value']}');
              });
              callback('${username.text} - ${password.text}');
            },
            icon: const Icon(Icons.login),
